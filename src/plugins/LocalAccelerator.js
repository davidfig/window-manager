/**
 * @typedef {object} LocalAcceleratorOptions
 * @property {HTMLElement} div
 */

 /** @type {LocalAcceleratorOptions} */
const LocalAcceleratorOptionsDefault = {
    div: document.body
}
...
export class LocalAccelerator
{
    ...
    constructor(options)
    {
        this.menuKeys = {}
        this.keys = {}
        this.options = Object.assign({}, LocalAcceleratorOptionsDefault, options)
        this.options.div.addEventListener('keydown', e => this.keydown(LocalAccelerator, e))
        this.options.div.addEventListener('keyup', e => this.keyup(LocalAccelerator, e))
    }
    ...
    {
        let key = ''
        const codes = this.prepareKey(keyCode)
    ...
    {
        const keys = this.prepareKey(keyCode)
        for (let key of keys)
        {
            this.keys[key] = (e) =>
            {
    ...
    keydown(accelerator, e)
    {
        ...
        {
            this.menuKeys[keyCode](e, LocalAccelerator)
        }
        else if (this.keys[keyCode])
        {
            this.keys[keyCode](e, LocalAccelerator)
        }
    }
}